...
create() {
    ...
    this.add.image(0,0, 'bakgrunn5').setScale(0.75).setOrigin(0.02,0); 
    ...
    gameState.startText = this.add.text(550, 300, 'Final Level!', { fontSize: '60px', fill: '#ff0000', fontFamily: 'Rock Kapak'  }).setOrigin(0.5);
    ...
    gameState.enemy1 = {
        sprite: this.add.sprite(760, 340, 'enemy5').setScale(0.30).setFlipX(true).setInteractive(),        
        health: 210,
        healthMax: 210,
        strength: 0, 
        armor: 0,
...
function drawCards(numCards) {
    gameState.yourTurnText = self.add.text(540, 300, 'Your turn!', { fontSize: '60px', fill: '#ff0000' }).setOrigin(0.5);
    gameState.playerPoisonText = self.add.text(550, 350, '', { fontSize: '25px', fill: '#ff0000' }).setOrigin(0.5);
    
    if (gameState.player.poison > 0) {
        gameState.player.health = Math.max(1, gameState.player.health - gameState.player.poison)
        gameState.playerPoisonText.setText(`-${gameState.player.poison} HP from poison`);
...
function startEnemyTurn() {
    ...
    gameState.enemyPoisonText = self.add.text(590, 350, '', { fontSize: '25px', fill: '#ff0000' }).setOrigin(0.5);

    if (gameState.enemy1.poison > 0) {
        gameState.enemy1.health = Math.max(1, gameState.enemy1.health - gameState.enemy1.poison)
        gameState.enemyPoisonText.setText(`-${gameState.enemy1.poison} HP from poison`);
...
function updateEnemyActions() {
    enemyActions = [ 
        {key: 'damage5', damage: 0, magic: 22, poison: 0, heal: 0, poisonRemove: 0, strength: 0, armor: 0, text: 'Deals 22 magic damage', probability: 0.175 + ((gameState.enemy1.health === gameState.enemy1.healthMax) ? 0.20 : 0) / 6},
        {key: 'damage10', damage: 23, magic: 0, poison: 0, heal: 0, poisonRemove: 0, strength: 0, armor: 0, text: 'Deals 24 damage', probability: 0.2 + ((gameState.enemy1.health === gameState.enemy1.healthMax) ? 0.20 : 0) / 6},
        {key: 'damage15', damage: 27, magic: 0, poison: 0, heal: 0, poisonRemove: 0, strength: 0, armor: 0, text: 'Deals 27 damage', probability: 0.15 + ((gameState.enemy1.health === gameState.enemy1.healthMax) ? 0.20 : 0) / 6},
        ...
        {key: 'heal20', damage: 0, magic: 0, poison: 0, heal: 25, poisonRemove: 0, strength: 0, armor: 0, text: 'Heals 27 HP', probability: (gameState.enemy1.health === gameState.enemy1.healthMax) ? 0 : 0.05},
        {key: 'strength1', damage: 0, magic: 0, poison: 0, heal: 0, poisonRemove: 0, strength: 5, armor: 0, text: 'Gains 5 strenght', probability: 0.125 + ((gameState.enemy1.health === gameState.enemy1.healthMax) ? 0.20 : 0) / 6},
        {key: 'armor1', damage: 0, magic: 0, poison: 0, heal: 0, poisonRemove: 0, strength: 0, armor: 5, text: 'Gains 5 armor', probability: 0.1 + ((gameState.enemy1.health === gameState.enemy1.healthMax) ? 0.20 : 0) / 6},
        {key: 'poison1', damage: 0, magic: 0, poison: 7, heal: 0, poisonRemove: 0, strength: 0, armor: 1, text: 'Deals 7 poison', probability: 0.05 + ((gameState.enemy1.health === gameState.enemy1.healthMax) ? 0.20 : 0) / 6}
    ];
}
